...
async def ensure_user_exists(user_id: str) -> bool:
    """Ask auth-service to ensure a Google user exists before saving BAITOAN."""
    if not user_id:
        return False

    def call_auth_service():
        response = requests.post(
            f"{settings.AUTH_SERVICE_URL.rstrip('/')}/auth/ensure-google-user",
            json={"googleId": user_id},
            timeout=15,
        )
    ...
    try:
        data = await asyncio.to_thread(call_auth_service)
        return bool(data.get("success"))
    except requests.RequestException as e:
        print(f"Warning: auth-service unavailable for user {user_id}: {e}")
        return False
...
async def upload_and_save_problem(
    ...
    try:
        ...
        try:
            if user_id:
                try:
                    user_ready = await ensure_user_exists(user_id)
                    if not user_ready:
                        user_id = None
                except Exception as e:
                    print(f"Warning: Could not verify user {user_id}: {e}")
                    user_id = None
...
# ============================================================
# BƯỚC 2: Render 3D bằng Gemini commands (TRƯỚC khi giải toán)
# ============================================================
